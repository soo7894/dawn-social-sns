...
export default function Home() {
  ...
  return <main className="app-shell">
    <aside className="sidebar"><button className="brand" onClick={() => selectTab("home")}>dawn<span>.</span></button><nav>{navItems.map(({ id, label, Icon }) => <button key={id} className={`nav-item ${tab === id ? "active" : ""}`} onClick={() => selectTab(id)} aria-label={label}><Icon className="nav-icon" />{label}{id === "activity" && data.notifications.some((item) => !item.read) ? <i className="unread-dot" /> : null}</button>)}</nav><a href="?admin=1" className="admin-entry"><IoGridOutline className="nav-icon" />관리자 확인</a><div className="sidebar-bottom"><button className="account-mini" onClick={() => selectTab("profile")}><img className="avatar-safe" src={data.user.avatar} onError={fallbackAvatar} alt="내 프로필" /><span><strong>{data.user.name}</strong><small>@{data.user.username}</small></span></button></div></aside>
